...
/*  Repartition des utilisatrices selon leurs situations matrimoniales */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=FQmarital_status DISPLAY=LABEL
  /TABLE BY FQmarital_status [C][LAYERPCT.COUNT PCT40.1]
  /CATEGORIES VARIABLES=FQmarital_status ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.

     /* Selon leurs groupe d'âge */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=AgeGr DISPLAY=LABEL
  /TABLE BY AgeGr [LAYERPCT.COUNT PCT40.1]
  /CATEGORIES VARIABLES=AgeGr ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.

           /* Selon leurs niveau d'éducation */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=school DISPLAY=LABEL
  /TABLE BY school [LAYERPCT.COUNT PCT40.1]
  /CATEGORIES VARIABLES=school ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.

 /* selon le niveau d'éducation et le groupe d'âge */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=school AgeGr DISPLAY=LABEL
  /TABLE school [C] BY AgeGr [C][LAYERPCT.COUNT PCT40.1]
  /CATEGORIES VARIABLES=school AgeGr ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.

       /* selon le niveau d'éducation et la situation matrimoniale */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=school FQmarital_status DISPLAY=LABEL
  /TABLE school [C][LAYERPCT.COUNT PCT40.1] BY FQmarital_status [C]
  /CATEGORIES VARIABLES=school FQmarital_status ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.

     /* selon la situation matrimoniale et le groupe d'âge */

* Tableaux personnalisés.
CTABLES
  /VLABELS VARIABLES=AgeGr FQmarital_status DISPLAY=LABEL
  /TABLE AgeGr [C] BY FQmarital_status [C][LAYERPCT.COUNT PCT40.1]
  /CATEGORIES VARIABLES=AgeGr FQmarital_status ORDER=A KEY=VALUE EMPTY=INCLUDE
  /CRITERIA CILEVEL=95.
...
**********************3)Taux de prévalence contraceptive des adolescentes et jeunes (toutes ou en union)   **************
...
/*En fonction du niveau de vie du ménage */
...
/*En fonction du niveau d'étude de la femme */
